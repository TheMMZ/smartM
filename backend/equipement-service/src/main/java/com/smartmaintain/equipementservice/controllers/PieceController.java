...
public class PieceController {
    ...
    @PostMapping
    public ResponseEntity<Piece> createPiece(@Valid @RequestBody Piece piece) {
        return ResponseEntity.status(HttpStatus.CREATED).body(pieceService.createPiece(piece));
    }

    @PutMapping("/{id}")
    public ResponseEntity<Piece> updatePiece(@PathVariable UUID id, @Valid @RequestBody Piece piece) {
