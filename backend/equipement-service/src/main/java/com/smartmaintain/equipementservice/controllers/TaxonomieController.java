...
public class TaxonomieController {
    ...
    @PostMapping
    public Taxonomie createTaxonomie(@Valid @RequestBody Taxonomie taxonomie) {
        return taxonomieService.saveTaxonomie(taxonomie);
    }

    @PutMapping("/{id}")
    public ResponseEntity<Taxonomie> updateTaxonomie(@PathVariable Long id, @Valid @RequestBody Taxonomie taxonomieDetails) {
